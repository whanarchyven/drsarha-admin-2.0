...
export default function EditNewsForm({ news }: EditNewsFormProps) {
  const initialValues: News = {
    ...
    doi: news.doi,
    meta: {
      isPublished: news.meta.isPublished,
      isIndexed: news.meta.isIndexed,
      hasDevComment: news.meta.hasDevComment,
      isDeleted: news.meta.isDeleted,
      hasTranslation: news.meta.hasTranslation,
      isClinicalCase: news.meta.isClinicalCase,
    },
    subcategory: news.subcategory,
    summary: news.summary,

    references: news.references,
    parserIteration: news.parserIteration,
  };
  ...
  const handleTranslate = async () => {
    ...
    if (res.data && (res.data as any).task_id) {
      ...
      const checkTask = async () => {
        ...
        const res = await eden.editor.news
    ...
    <div>
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-4">
          ...
          <Badge variant={news.meta.isPublished ? 'success' : 'destructive'}>
            {news.meta.isPublished ? 'Опубликовано' : 'Не опубликовано'}
          </Badge>
