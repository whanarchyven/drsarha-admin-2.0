...
const NewsCard = (props: NewsCardProps) => {
  ...
  const handleDelete = async () => {
    const res = await eden.editor.news({ id: props._id as string }).delete();
    if (res?.error) {
  ...
  const handlePublish = async () => {
    const res = await eden.editor.news({ id: props._id as string }).patch({
      isPublished: true,
    });
    if (res?.error) {
      toast.error(res.error.value.message);
    ...
    <>
      <Card className="w-full overflow-hidden border-cBlack border-2">
        ...
        <CardFooter className="flex justify-end gap-2">
          ...
          <Link href={`/news/${props._id}/edit`} target="_blank">
