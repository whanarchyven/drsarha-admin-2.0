...
export function SubscriberCard({
  ...
  const tariff =
    new Date(subscriber.subscribeTill) > new Date()
      ? subscriber.tariff
      : 'free';
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-6">
        <div className="flex justify-between">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            onClick={() => onOpenDetails(subscriber)}>
        ...
        <div className="space-y-2 mb-4">
          <div className="flex justify-between">
            ...
            <Badge className={getTariffBadgeColor(tariff)}>
              {tariff.toUpperCase()}
            </Badge>
          </div>
          <div className="flex justify-between">
            ...
            <span
              className={clsx(
                'text-sm font-bold',
                tariff === 'free' && 'text-red-500'
              )}>
              {formatDate(subscriber.subscribeTill)}
            </span>
          </div>
        </div>
      </CardContent>
      ...
      <CardFooter className="grid grid-cols-2 gap-2 p-6 pt-0">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          onClick={() => onPasswordChange(subscriber)}>
        ...
        <Button
          variant="success"
          size="sm"
          onClick={() => onUpdateSubscription(subscriber)}>
